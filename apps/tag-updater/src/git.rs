...
pub fn clone(
    ...
    let repository = builder
        .clone(url, filepath)
        .wrap_err_with(|| eyre!("Failed to clone repository at '{url}' into {filepath:?}"))?;
...
pub fn fetch<'a>(
    ...
    cb.credentials(|_url, username_from_url, _allowed_types| {
        git2::Cred::ssh_key_from_memory(username_from_url.unwrap(), None, private_key, None)
    });
...
pub fn push(
    ...
    let refs = &[reference];
    remote.push(refs, Some(&mut push_options)).unwrap();
